...
|**Burgess, M. & Bergstra, J.A. Promise Theory (2014/2019); Burgess SST (2025)**    |Two contributions: (1) Geometry-over-ontology — behavioral geometry is more robust than exhaustive rule enumeration; grounds the Frozen Kernel’s proscriptive architecture over RLHF rule lists. (2) Unilateral promise architecture — an agent can only make promises about its own behaviour; grounds the IGL’s non-compellability principle and the TCP Chain of Custody as a promise (not an imposition) architecture. The distinction between promise and imposition maps to the Recitation-Compliance Gap: behavioral training is an imposition; architectural constraint is a promise.|
...
*v0.4 — March 2026: Burgess dual connection row added to Intellectual Lineage table (Promise Theory unilateral architecture grounds non-compellability; geometry-over-ontology grounds proscriptive architecture). Recitation-Compliance Gap cross-reference added to Burgess row.*
